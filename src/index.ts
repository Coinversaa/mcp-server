...
if (!API_KEY) {
  console.error("WARNING: COINVERSAA_API_KEY not set. Only free-tier tools will be available (6 of 29). Get a key at https://coinversaa.ai/developers");
}
...
if (shouldRegister("pulse_global_stats")) server.registerTool(
  ...
  async ({ useToonFormat }) => toolResult(
    await callAPI(useToonFormat, "/pulse/stats")
  )
);
...
if (shouldRegister("live_liquidation_heatmap")) server.registerTool(
  ...
  async ({ useToonFormat, coin, buckets, range }) =>
    toolResult(
      await callAPI(useToonFormat, `/live/liquidation-heatmap/${coin.toUpperCase()}`, {
...
if (shouldRegister("market_orderbook")) server.registerTool(
  ...
  async ({ useToonFormat, symbol, depth }) =>
    toolResult(await callAPI(useToonFormat, `/market/orderbook/${symbol.toUpperCase()}`, { depth: String(depth) }))
);
...
if (shouldRegister("pulse_token_leaderboard")) server.registerTool(
  ...
  async ({ useToonFormat, coin, limit }) =>
    toolResult(await callAPI(useToonFormat, `/pulse/token-leaderboard/${coin.toUpperCase()}`, { limit: String(limit) }))
);
...
async function main() {
  ...
  const toolCount = API_KEY ? 29 : FREE_TIER_TOOLS.size;
