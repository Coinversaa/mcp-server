...
async function main() {
  ...
  console.error(
    `Coinversa Pulse MCP server running on stdio (${COINVERSA_TOTAL_TOOL_COUNT - hiddenToolsFromEnv().size} tools available${hiddenToolsFromEnv().size > 0 ? `, ${hiddenToolsFromEnv().size} withheld by COINVERSAA_HIDDEN_TOOLS` : ""}; backend enforces API-key tier)`
  );
}
