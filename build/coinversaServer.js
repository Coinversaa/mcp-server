...
export function hiddenToolsFromEnv(raw = process.env.COINVERSAA_HIDDEN_TOOLS) {
...
export const NEW_TO_LEGACY_TIER = {
 ...
 * through untouched.
 */
export function capArray(obj, key, limit, opts = {}) {
    const arr = obj?.[key];
    if (!Array.isArray(arr))
        return { ...obj, totalCount: 0, truncated: false };
    ...
    const out = { ...obj, [key]: sliced, totalCount: total, truncated };
 ...
 */
export function bucketSeries(rows, bucketMs, ts, score) {
    if (!Array.isArray(rows) || bucketMs <= 0)
        return rows;
    const best = new Map();
 ...
 */
export function filterCohortRows(rows, tierType, tier) {
...
export function sortCohortRows(rows, timeKey) {
    if (!Array.isArray(rows))
        return rows;
    const cmp = (a, b) => {
...
export const RISK_HISTORY_SECTIONS = ["oiHistory", "longShortHistory", "cohortBiasHistory", "candleHistory", "markDislocations", "liquidations"];
export const RISK_HISTORY_DEFAULT_SECTIONS = RISK_HISTORY_SECTIONS.filter((s) => s !== "markDislocations");
export function createCoinversaServer(options = {}) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (response.status === 429) {
                    ...
                    if (body429?.retryAfterSeconds)
                        parts.push(`Retry in ~${body429.retryAfterSeconds}s.`);
                    if (body429?.upgrade_url)
                        parts.push(`Higher limits are available on a paid plan — upgrade at ${body429.upgrade_url} (or call pulse_my_plan to see your current tier and every plan's limits).`);
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 403 && body?.required_tier) {
                        ...
                        throw new Error(`This feature${tool} requires the ${body.required_tier} tier — the API key in use is on the ${cur} tier.${upgrade} ` +
    ...
    const COVERAGE_DATASETS = ["trades", "builder_ledger", "census", "hip4", "liquidations", "lifecycles", "cohort_history", "book"];
    /** Extract the first YYYY-MM-DD date following "history begins" in a builder dataNotes string. */
    function ledgerStartFromDataNotes(notes) {
    ...
    const COVERAGE_CALL = { timeoutMs: 15_000, retries: 2 };
    ...
    const coverageSources = {
        ...
        builder_ledger: async () => {
            ...
            if (!verified)
                notes.push("No 'verified' ledger stamp in the response (ledger table absent or empty).");
            if (!coverage)
                notes.push("Attribution coverage rollup not provisioned yet (verified.coverage is null).");
            return {
        ...
        hip4: async () => {
            ...
            const latest = outcomes.map((o) => o?.lastTraded).filter((t) => typeof t === "string").sort().pop() ?? null;
            return {
                dataset: "hip4",
...
- The list_markets tool accepts an optional dex filter to narrow results`;
    ...
    if (shouldRegister("live_coin_risk_history"))
        server.registerTool("live_coin_risk_history", {
            ...
            const wanted = new Set(include ?? RISK_HISTORY_DEFAULT_SECTIONS);
            const result = {};
            for (const [k, v] of Object.entries(history ?? {})) {
                if (RISK_HISTORY_SECTIONS.includes(k))
                    continue;
                result[k] = v;
            }
            for (const section of RISK_HISTORY_SECTIONS) {
                ...
                if (section === "markDislocations") {
                    const raw = Array.isArray(history?.markDislocations) ? history.markDislocations : [];
    ...
    if (shouldRegister("pulse_cohort_bias_history"))
        server.registerTool("pulse_cohort_bias_history", {
            ...
            const all = Array.isArray(data) ? data : Array.isArray(data?.rows) ? data.rows : [];
    ...
    if (shouldRegister("live_cohort_bias_history"))
        server.registerTool("live_cohort_bias_history", {
            ...
            if (tier)
                params.tier = normalizeTier(tier);
    ...
    if (shouldRegister("hip4_trader_outcomes"))
        server.registerTool("hip4_trader_outcomes", {
            ...
            const notional = (o) => (Number(o?.grossBoughtUsdh) || 0) + (Number(o?.grossSoldUsdh) || 0);
            const outcomes = Array.isArray(data?.outcomes) ? [...data.outcomes] : [];
    ...
    if (shouldRegister("book_levels"))
        ...
        }, async ({ useToonFormat, coin, depth }) => toolResult(await callAPI(useToonFormat, bookPath("levels", coin), { depth: String(depth) })));
    // ─── Data coverage ─────────────────────────────────
    if (shouldRegister("data_coverage"))
        server.registerTool("data_coverage", {
            title: "Data Coverage",
            ...
            const wanted = dataset ? [dataset] : [...COVERAGE_DATASETS];
            ...
            for (const [i, d] of wanted.entries()) {
                if (i > 0)
                    await new Promise((r) => setTimeout(r, COVERAGE_PACE_MS));
                try {
